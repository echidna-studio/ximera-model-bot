...
# -*- coding: utf-8 -*-
"""
Ximera Model Agent — Telegram bot.
...
STEPS = [
    ...
    ("params",  "Ваш рост и вес (например: 175 / 52)"),
...
WELCOME = (
    "<b>«Ximera Model Agent»</b>\n"
    "Привет! Рад видеть тебя здесь. ✨\n\n"
    "Я модельный агент, и моя главная задача — сделать так, чтобы ты зарабатывала много "
    "и в комфортных условиях. Я беру на себя всю рутину и организацию, чтобы ты могла "
    "сфокусироваться на главном.\n\n"
    "Здесь нет пустых обещаний. Только прозрачная работа и моя личная заинтересованность "
    "в твоём финансовом росте.\n\n"
    "<b>Почему со мной комфортно и выгодно:</b>\n\n"
    "🚫 <b>Никакой пошлости:</b> я работаю только с адекватными проектами, эстетичным "
    "контентом и проверенными брендами. Никаких съёмок 18+, эротики и сомнительных форматов.\n\n"
    "💰 <b>Честный процент:</b> максимально прозрачные выплаты без скрытых комиссий.\n\n"
    "🎓 <b>Обучение и забота:</b> даже если у тебя нет опыта, я дам все необходимые "
    "инструкции и буду на связи 24/7.\n\n"
    "🎯 <b>Индивидуальный подход:</b> подбираем формат и гибкий график под твои личные цели.\n\n"
    "Чтобы я мог предложить тебе лучшие условия, давай немного познакомимся. Это займёт "
    "буквально пару минут и ни к чему тебя не обязывает.\n\n"
    "Жми кнопку «Оставить заявку» ниже, и начнём! 👇"
)

FORM_START = (
    "Отличный шаг! 🚀\n\n"
    "Сейчас я задам тебе несколько простых вопросов. Отвечай максимально честно — "
    "эта информация нужна только для того, чтобы подобрать идеальный для тебя вектор развития."
)

FORM_DONE = (
    "✅ <b>Спасибо! Ваша анкета отправлена.</b>\n\n"
    "Я лично изучу твою анкету и свяжусь с тобой в ближайшее время, чтобы обсудить детали, "
    "ответить на все твои вопросы и рассказать о дальнейших шагах.\n\n"
    "Включи уведомления, чтобы не пропустить моё сообщение. До скорой связи! 🔥"
)
...
def app_card(a, for_admin=True):
    ...
    t = (f"🆕 <b>Заявка #{a['id']}</b>  ·  {status}\n\n"
         ...
         f"📏 <b>Рост / вес:</b> {e(a['data'].get('params'))}\n"
...
def finish_form(uid, chat_id, user):
    ...
    db_save()
    send(chat_id, FORM_DONE, RKB_REMOVE)
    send_app_to_admin(a)
...
def on_message(m):
    ...
    if text.startswith("/start"):
        ...
        if is_admin(uid):
            send(chat_id,
                 "👑 <b>Вы админ</b>\n\n"
                 "Вы администратор бота <b>Ximera Model Agent</b>.\n"
